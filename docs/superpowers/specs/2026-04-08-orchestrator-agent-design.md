...
4. 检查 Orchestrator 收件箱: ls company-ops/shared/messages/inbox-orchestrator/ 2>/dev/null
5. 如果 inbox 中有消息，读取并处理，写回复到 outbox-orchestrator，然后将原消息移到 processed/
...
2. **inbox/outbox 文件系统** — 结构化消息：通过 `shared/messages/` 目录传递完整的 JSON 消息和回复
...
2. 创建 `shared/messages/` 目录结构
3. 配置 CronCreate 定时任务（10 分钟间隔）
4. 测试监控循环
