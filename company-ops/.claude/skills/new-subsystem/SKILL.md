...
**触发条件：**
- 手动调用：`!new-subsystem <subsystem-name>`
...
# 或先创建基础模板，再完善
python scripts/init_subsystem.py 财务 --type=function
# 然后运行此 Skill 继续完善
/new-subsystem
```
...
调用 `init_subsystem.py` 创建基础目录结构：
- inbox/, outbox/, state/, data/, src/, scripts/, libs/, docs/, workspace/
- CLAUDE.md, SPEC.md, CONTRACT.yaml, CAPABILITIES.yaml, local-graph.json
...
| 4 | 决策权限 | CLAUDE.md |
| 5 | 服务定义 | CONTRACT.yaml, CAPABILITIES.yaml |
